...
public class WaypointList extends ObjectSelectionList<WaypointEntry> {

    private final FastFlightScreen screen;
    public WaypointList(FastFlightScreen screen, Minecraft mc, int width, int height, int y0, int y1, int itemHeight) {
    ...
    protected void renderList(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        for (int i = 0; i < getItemCount(); ++i) {
            ...
            if (bottom >= y0 && top <= y1) {
                WaypointEntry entry = getEntry(i);
                if (isSelectedItem(i)) {
    ...
    public void fillWaypoints(){
        ...
        Minecraft.getInstance().player.getCapability(PlayerAbilitiesProvider.PLAYER_ABILITIES).ifPresent(cap -> {
            if(cap.getAscendantType() == AscendantType.EAGLE){
                ...
                ArrayList<WaypointNBTEntry> list = ascendant.getWaypointList();
                list.forEach(point -> {
                    addEntry(new WaypointEntry(this, point.x, point.z, point.name, point.dimension));
                });
            }
        });
    }
    ...
    public FastFlightScreen getParentScreen() {
        return screen;
    }
}
