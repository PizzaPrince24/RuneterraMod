...
public class ScorpionAscensionPendant extends Item implements ICurioItem {
    public ScorpionAscensionPendant(Properties pProperties) {
        super(pProperties);
    }
    @Override
    public void onEquip(SlotContext slotContext, ItemStack prevStack, ItemStack stack) {
        slotContext.entity().getCapability(PlayerAbilitiesProvider.PLAYER_ABILITIES).ifPresent(cap -> {
            cap.ascend((Player) slotContext.entity(), AscendantType.SCORPION);
        });
